...
常用 ASCII 快捷键读取物理按键，不受中英文输入法状态影响；即使输入法正在选词，数字快捷键也不会被吞掉。桌面版会在关卡界面关闭文本组合，避免选词框出现或闪烁；离开关卡界面后会自动恢复文本输入。
...
字母、数字、Space、Tab、Enter 和常见美式键位标点通过游戏已有的 `KeyDown` 事件识别。标点键位可能随键盘布局变化；实现细节见 [键盘输入与输入法](keyboard-input.md)。
...
本修改器已适配中文等常见非英文输入法下的快捷键，并在桌面版关卡界面关闭文本组合，防止输入法选词框出现或闪烁。不同系统、键盘布局和输入法仍可能采用额外的按键拦截方式；如果快捷键仍然失效，请切换到英文输入法后再试。
